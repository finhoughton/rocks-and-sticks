...
from players import AIPlayer, MCTSPlayer, Player, RandomPlayer, applied_move
...
def play_self_play_game(player_factories: Iterable[Callable[[int], Player]], max_moves: int = 256) -> tuple[list[EncodedGraph], int | None]:
    players = [factory(i) for i, factory in enumerate(player_factories)]
    game = Game(players)
    trajectory: list[EncodedGraph] = []

    _randomize_start(game)
...
def _randomize_start(
    ...
    for _ in range(rollout_moves):
        ...
        if game.winner is not None and game.winner != mover.number:
            game.undo_move()
            break
        # avoid ending with an immediate win
        if game.winner is not None:
            game.undo_move()
            break
...
class BiasedRandomPlayer(RandomPlayer):
    """Random player with a slight bias toward stick moves to diversify play."""

    def get_move(self, game: Game) -> Move:
        moves = list(game.get_possible_moves(self))
        if not moves:
            return super().get_move(game)
        stick_moves = [m for m in moves if m.t in D.__members__]
        if stick_moves and self._rng.random() < 0.7:
            stick_moves.sort(key=move_sort_key)
            return self._rng.choice(stick_moves)
        return super().get_move(game)
...
def build_dataset(num_games: int, player_factories: Iterable[Callable[[int], Player]], max_moves: int = 256, augment_sym: bool = True) -> list[Data]:
    samples: list[Data] = []
    for _ in range(num_games):
        traj, winner = play_self_play_game(player_factories, max_moves=max_moves)
        for enc in traj:
...
def main() -> None:
    ...
    def player_factory(idx: int) -> Player:
        ...
        if args.use_mcts and idx % 2 == 0:
            return MCTSPlayer(idx, time_limit=0.10, n_rollouts=120)
    ...
    dataset = build_dataset(args.games, [player_factory, player_factory], max_moves=args.max_moves, augment_sym=args.augment_sym)
